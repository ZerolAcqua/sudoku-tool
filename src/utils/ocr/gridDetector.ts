...
function findOptimalLineSubset(lines: number[]): number[] | null {
  ...
  for (const strategy of strategies) {
    ...
    if (subset.length === expectedCount && validateLineSpacingWithGap(subset)) {
      console.log('[findOptimalLineSubset] 找到最优子集：移除[', strategy.start, ':', lines.length - strategy.end, ']')
      return subset
    }
  }
  ...
  for (const strategy of strategies) {
    ...
    if (subset.length === expectedCount) {
      ...
      if (gap && Math.abs(gap - targetGap) < tolerance) {
        console.log('[findOptimalLineSubsetWithGapConstraint] 找到匹配间距的子集，间距:', gap.toFixed(1))
        return subset
      }
    }
  }
...
function validateLineSpacingDetails(lines: number[]): number | null {
  ...
  console.log('[validateLineSpacingDetails] 基本间距:', baseGap.toFixed(1), '检测线条数:', lines.length)
  ...
  for (let i = 0; i < gaps.length; i++) {
    ...
    if (!isValidMultiple) {
      console.log(`[validateLineSpacingDetails] 间距 ${i} 异常: ${gap.toFixed(1)} (基本: ${baseGap.toFixed(1)})`)
      return null
    }
  }
...
function tryRemoveInterferingLinesWithGap(lines: number[]): number | null {
  ...
  for (const strategy of strategies) {
    ...
    if (subset.length === expectedCount) {
      ...
      if (gap) {
        console.log('[tryRemoveInterferingLinesWithGap] 策略:', strategy.name, '成功！间距:', gap.toFixed(1))
        return gap
      }
    }
  }
...
function buildRectFromLinesSquare(horizontalLines: number[], verticalLines: number[]): GridLocation {
  ...
  if (sizeRatio > 1.1) {
    console.log('[buildRectFromLinesSquare] ⚠️ 尺寸不均匀，宽/高比:', (rawWidth / rawHeight).toFixed(3), '将强制调整为正方形')
  }
  ...
  console.log('[buildRectFromLinesSquare] 原始尺寸:', rawWidth.toFixed(1), 'x', rawHeight.toFixed(1), '→ 正方形:', sideLength.toFixed(1), 'x', sideLength.toFixed(1))
  console.log('[buildRectFromLinesSquare] 最终边框:', rect, '线条数: ' + horizontalLines.length + 'x' + verticalLines.length)
  return rect
}
...
function buildRectFromLines(horizontalLines: number[], verticalLines: number[]): GridLocation {
  ...
  console.log('[buildRectFromLines] 直线检测成功 - 边框:', rect, '线条数: ' + horizontalLines.length + 'x' + verticalLines.length)
  return rect
}
